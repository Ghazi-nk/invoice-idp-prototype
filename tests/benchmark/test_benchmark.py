...
class TestBenchmarkWorker(unittest.TestCase):
    ...
    @patch('benchmark.check_acceptance', return_value=True)
    @patch('benchmark.is_match')
    @patch('benchmark.extract_invoice_fields_from_pdf')
    ...
    def test_process_task_success(self, mock_time, mock_file, mock_extract, mock_is_match, mock_check_acceptance):
        ...
        mock_extract.return_value = {"total": 100, "vendor": "TestCorp", "iban": "DE123"}
        ...
        self.assertEqual(summary['duration'], 15.5)
        ...
        self.assertEqual(summary['precision'], 1.0)  # tp=1, fp=1 -> 1/2 = 0.5... wait.
        # Let's trace metrics:
    ...
    @patch('benchmark.extract_invoice_fields_from_pdf', side_effect=Exception("OCR Failed"))
...
class TestBenchmarkFinalResults(unittest.TestCase):
    ...
    @patch('benchmark.os.path.exists', return_value=True)
    def test_generate_final_results_success(self, mock_exists):
        """Should correctly read a summary CSV and write aggregated results."""
        # Arrange
        mock_summary_data = (
            "invoice,pipeline,accuracy,precision,recall,f1,acceptance,duration\n"
            "inv1,engineA,1.0,1.0,1.0,1.0,1,10.0\n"
            "inv2,engineA,0.5,0.5,0.5,0.5,0,20.0\n"
            "inv1,engineB,0.8,0.8,0.8,0.8,1,5.0\n"
        )
        ...
        # Check header
        self.assertIn("pipeline,mean_accuracy,mean_precision,mean_recall,mean_f1,mean_duration,acceptance_rate",
                      written_content)
        # Check aggregated data for engineA
        # accuracy: (1.0+0.5)/2=0.75, precision: (1.0+0.5)/2=0.75, duration: (10+20)/2=15, acceptance: (1+0)/2=0.5
        self.assertIn("engineA,0.75,0.75,0.75,0.75,15.0,0.5", written_content)
        # Check aggregated data for engineB
        self.assertIn("engineB,0.8,0.8,0.8,0.8,5.0,1.0", written_content)

    @patch('benchmark.os.path.exists', return_value=False)
...
if __name__ == '__main__':
    ...
    with patch('benchmark.OUTPUT_SUMMARY_CSV', 'summary.csv'), \
            patch('benchmark.OUTPUT_DETAIL_CSV', 'details.csv'), \
            patch('benchmark.OUTPUT_RESULTS_CSV', 'results_ollama_model_tag.csv'):
