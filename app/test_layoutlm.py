#!/usr/bin/env python3
"""
Test für die LayoutLM OCR-Engine mit und ohne Bounding Box.
"""
...
from app.document_processing import layoutlm_process_pdf
from app.document_digitalization.pre_processing import standardize_ocr_output

def main():
    """Test für die LayoutLM OCR-Engine."""
    ...
    print("\n=== Test layoutlm_process_pdf ohne Bounding Box ===")
    try:
        result = layoutlm_process_pdf(sample_pdf, include_bbox=False)
        print(f"Erfolg! {len(result)} Seiten extrahiert")
        
        if result and len(result) > 0:
            for i, page in enumerate(result):
                preview = page[:100] + "..." if isinstance(page, str) and len(page) > 100 else page
    ...
    print("\n=== Test layoutlm_process_pdf mit Bounding Box ===")
    try:
        result_bbox = layoutlm_process_pdf(sample_pdf, include_bbox=True)
        print(f"Erfolg! {len(result_bbox)} Seiten extrahiert")
        
        if result_bbox and len(result_bbox) > 0:
            for i, page in enumerate(result_bbox):
                print(f"Seite {i+1}: {type(page)}")
                if isinstance(page, list) and len(page) > 0:
                    print(f"  {len(page)} Elemente gefunden")
                    first_item = page[0]
